    ...
    for i in X:
        for j in Y:
            if i != lb[0] and i != ub[0] and j != lb[1] and j != ub[1] and w(i, j, t) > 0:
                tmp_Region.append([i, j])
    # tmp_Region = np.asarray(tmp_Region, dtype=float)
    X_Region.append(tmp_Region)
    new_T.append([t])
...
for t in T:
    ...
    for x1, x2 in tmp_gamma:
        ...
        new_gamma.append([new_x1, new_x2])
    # new_gamma = np.asarray(new_gamma, dtype=float)
    X_gamma.append(new_gamma)
...
scipy.io.savemat(root_path + '/origin2.mat',
                 {'X_Region': X_Region, 'X_gamma': X_gamma, 'T': new_T})
