...
class PINN(nn.Module):
    ...
    def optimize_one_epoch(self):
        ...
        len_X_Region = self.X_Region.shape[0]
        len_X_gamma = self.X_gamma.shape[0]
        # 打乱数据
        X_Region_idx = list(range(len_X_Region))
        X_gamma_idx = list(range(len_X_gamma))
        np.random.shuffle(X_Region_idx)
        np.random.shuffle(X_gamma_idx)

        Region_Batch_Size = 159820
        ...
        while iter*Region_Batch_Size < len_X_Region or iter*gamma_Batch_Size < len_X_gamma:
            ...
            if iter*Region_Batch_Size >= len_X_Region:
                ...
                target_X_Region = self.X_Region[X_Region_idx[iter *
            ...
            if iter*gamma_Batch_Size >= len_X_gamma:
                ...
                target_X_gamma = self.X_gamma[X_gamma_idx[iter *
