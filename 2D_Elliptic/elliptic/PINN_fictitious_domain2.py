...
class PINN(nn.Module):
    ...
    # 设置网络是否可导
    def change_psi1_grad_state(self, grad=True):
        num_layers = len(self.psi1_weights)
        for l in range(num_layers):
            W = self.psi1_weights[l]
            b = self.psi1_biases[l]
            W.requires_grad = grad
            b.requires_grad = grad

    def change_psi2_grad_state(self, grad=True):
        num_layers = len(self.psi2_weights)
        for l in range(num_layers):
            W = self.psi2_weights[l]
            b = self.psi2_biases[l]
            W.requires_grad = grad
            b.requires_grad = grad

    def change_v_grad_state(self, grad=True):
        num_layers = len(self.v_weights)
        for l in range(num_layers):
            W = self.v_weights[l]
            b = self.v_biases[l]
            W.requires_grad = grad
    ...
    # Omega区域损失
    def loss_psi1(self, training_type=0):
        ...
        if training_type == 0:
            ...
            equation1 = self.alpha * psi - self.mu * \
            ...
            equation2 = self.alpha * psi1 - self.mu * \
                (psi1_x1_x1 + psi1_x2_x2) - v

            loss = loss + self.loss_func(equation1) + self.loss_func(equation2)

        return loss

    # omega区域损失
    def loss_psi2(self, training_type=0):

        loss_J = torch.tensor(0.0, dtype=torch.float32).to(self.device)
        loss_J.requires_grad_()

        loss_psi2 = torch.tensor(0.0, dtype=torch.float32).to(self.device)
        loss_psi2.requires_grad_()
        ...
        if training_type == 1:
            ...
            loss_psi2 = loss_psi2 + self.loss_func(equation2)
            ...
            loss_J = loss_J + self.loss_func(equation2)
            ...
            loss_J = loss_J + self.loss_func(equation1)

            # # 最小化J 用Monte-Carlo 求积分
            # loss_J = loss_J + torch.pi/2 * self.a * self.b / x2.size(0) * (self.alpha * torch.sum(
            #     (psi2-psi1)**2) + self.mu*torch.sum((psi2_x1-psi1_x1)**2 + (psi2_x2-psi1_x2)**2))

            # 改为MSE
            loss_J = loss_J + self.loss_func(psi2, psi1)

        return loss_psi2, loss_J
    ...
    # 内边界损失
    def loss_gamma(self, training_type=0):
        loss = torch.tensor(0.0, dtype=torch.float32).to(self.device)
        loss.requires_grad_()
        if training_type == 1:
            x1 = self.X_gamma[:, 0:1]
            ...
            # 求单位法向量(n1,n2)
            # psi2_sign = -1 * \
            #     torch.sign(psi2_x1*(x1-self.G1) + psi2_x2*(x2-self.G2))
            ...
            # psi1_x1 = self.compute_grad(psi1, x1)
            # psi1_x2 = self.compute_grad(psi1, x2)
            # psi1_sign = torch.sign(psi1_x1*(x1-self.G1) + psi1_x2*(x2-self.G2))
            # sqrt_ = torch.sqrt(psi1_x1**2 + psi1_x2**2)
            # n1 = psi1_x1 / sqrt_ * psi1_sign
            # n2 = psi1_x2 / sqrt_ * psi1_sign
    ...
    # 训练一次
    def optimize_one_epoch(self, training_type=0):
        # type = 0 train psi1
        # type = 1 train psi2
        # type = 2 train v
        ...
        loss_psi1 = self.loss_psi1(training_type=training_type)
        loss_psi2, loss_J = self.loss_psi2(training_type=training_type)
        loss_Gamma = self.loss_Gamma(use_Gamma=False)
        loss_gamma = self.loss_gamma(training_type=training_type)

        # 权重
        alpha_psi1 = 1
        alpha_psi2, alpha_J = 1, 1
        alpha_Gamma = 1
        alpha_gamma = 1

        self.loss = loss_psi1 * alpha_psi1 + \
            loss_psi2 * alpha_psi2 + loss_J*alpha_J +\
        ...
        if np.remainder(self.nIter, loss_remainder) == 0:
            # 打印常规loss
            loss_psi1 = self.detach(loss_psi1)
            loss_psi2 = self.detach(loss_psi2)
            loss_J = self.detach(loss_J)
            loss_Gamma = self.detach(loss_Gamma)
            loss_gamma = self.detach(loss_gamma)

            log_str = str(self.optimizer_name) + ' Iter ' + str(self.nIter) + ' training_type ' + str(training_type) + ' Loss ' +\
                str(loss) + ' loss_psi1 ' + str(loss_psi1) + ' loss_psi2 ' + str(loss_psi2) +\
                ' loss_J ' + str(loss_J) +\
    ...
    def train_Adam(self, optimizer, Adam_scheduler, nIter, psi1Iter, psi2Iter, vIter):
        self.optimizer = optimizer
        self.optimizer_name = 'Adam'
        self.scheduler = Adam_scheduler
        for it in range(nIter):
            for v_i in range(vIter):
                # 训练v网络
                self.change_psi1_grad_state(False)
                self.change_psi2_grad_state(False)
                self.change_v_grad_state(True)
                self.optimize_one_epoch(training_type=2)
                self.optimizer.step()
                if self.scheduler is not None:
                    self.scheduler.step()
            for psi1_i in range(psi1Iter):
                # 训练psi网络
                self.change_psi1_grad_state(True)
                self.change_psi2_grad_state(False)
                self.change_v_grad_state(False)
                self.optimize_one_epoch(training_type=0)
                self.optimizer.step()
                if self.scheduler is not None:
                    self.scheduler.step()
            for psi2_i in range(psi1Iter):
                # 训练psi网络
                self.change_psi1_grad_state(False)
                self.change_psi2_grad_state(True)
                self.change_v_grad_state(False)
                self.optimize_one_epoch(training_type=1)
                self.optimizer.step()
                if self.scheduler is not None:
                    self.scheduler.step()
