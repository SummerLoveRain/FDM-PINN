...
class PINN(nn.Module):
    ...
    def __init__(self, param_dict, train_dict):
        ...
        # 设置使用设备:cpu, cuda
        lb, ub, self.psi1_layers, self.v_layers, self.device, self.path, self.root_path = self.unzip_param_dict(
        ...
        self.w_lb = self.data_loader(
            np.array([self.G1-self.a, self.G2-self.b]))
        self.w_ub = self.data_loader(
            np.array([self.G1+self.a, self.G2+self.b]))
        # 训练数据：坐标
        ...
        self.psi1_weights, self.psi1_biases = self.initialize_NN(
            self.psi1_layers)
        self.v_weights, self.v_biases = self.initialize_NN(
            self.v_layers)
    ...
    def net_v(self, x1, x2):
        ...
        v = self.neural_net(X, self.v_weights, self.v_biases)
        return v
    ...
    def loss_omega(self):
        ...
        loss_omega = self.loss_func(equation1, v)
    ...
    # 内边界损失
    def loss_gamma(self, use_f=False):
        x1 = self.X_gamma[:, 0:1]
        x2 = self.X_gamma[:, 1:2]
        # 计算psi1
        psi1 = self.net_psi1(x1, x2)
        ...
        equation = self.mu*(n1*psi1_x1 + n2*psi1_x2 + psi1 /
                            self.ls) - self.g1(x1, x2, n1, n2)
        loss_gamma = self.loss_func(equation)

        loss_v = torch.tensor(0.0, dtype=torch.float32).to(self.device)
        loss_v.requires_grad_()
        if use_f:
            # 计算v
            v = self.net_v(x1, x2)
            # 计算v = f
            ...
            loss_v = self.loss_func(f, v)

        return loss_gamma, loss_v
    ...
    def optimize_one_epoch(self):
        ...
        loss_gamma, loss_v = self.loss_gamma(use_f=True)
        ...
        loss_omega = loss_omega / self.N

        self.loss = loss_Omega * alpha_Omega + \
            loss_omega * alpha_omega +\
            loss_Gamma * alpha_Gamma + loss_gamma * alpha_gamma + loss_v * alpha_v
        # 反向传播
        self.loss.backward()
        # 运算次数加1
