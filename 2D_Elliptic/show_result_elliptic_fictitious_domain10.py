...
if __name__ == "__main__":
    ...
    # 加载各个区域的坐标数据
    TIME_STR = '20220510_021249'
    TASK_NAME = 'task_pinn_fictitious_domain10'
    ...
    ls = 0.1
    # f(x1, x2)
    def f(x1, x2): return (alpha+8*np.pi**2*mu)*psi(x1, x2)
    def g0(x1, x2): return psi(x1, x2)

    def g1(x1, x2, n1, n2): return mu*((n1*2*np.pi*np.cos(2*np.pi*x1)*np.cos(2*np.pi*x2)
                                       - n2*2*np.pi*np.sin(2*np.pi*x1)*np.sin(2*np.pi*x2))+psi(x1, x2)/ls)
