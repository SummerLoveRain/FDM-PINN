...
class PINN(nn.Module):
    ...
    def loss_Omega(self):
        ...
        loss_Omega = torch.tensor(0.0, dtype=torch.float32).to(self.device)
        loss_Omega.requires_grad_()

        equation = self.alpha * psi1 - self.mu * \
            (psi1_x1_x1 + psi1_x2_x2) - f
        loss_Omega = loss_Omega + self.loss_func(equation)
        return loss_Omega
    ...
    # 内边界损失
    def loss_gamma(self, use_v=False):
        x1 = self.X_gamma[:, 0:1]
        x2 = self.X_gamma[:, 1:2]
        # 计算psi1
        psi1 = self.net_psi1(x1, x2)
        ...
        loss_gamma = torch.tensor(0.0, dtype=torch.float32).to(self.device)
        loss_gamma.requires_grad_()
        equation = self.mu*(n1*psi2_x1 + n2*psi2_x2 - psi1 /
                            self.ls) + g1
        loss_gamma = loss_gamma + self.loss_func(equation)
        ...
        if use_v:
            f = self.f(x1, x2)
            v = self.net_v(x1, x2)
            loss_v = loss_v + self.loss_func(v, f)
    ...
    def optimize_one_epoch(self):
        ...
        loss_gamma, loss_v = self.loss_gamma(use_v=True)

        # 权重
        alpha_Omega = 1
        alpha_omega1, alpha_omega2, alpha_J = 1, 1, 1
        alpha_Gamma = 1
        alpha_gamma, alpha_v = 1, 1

        self.loss = loss_Omega * alpha_Omega + loss_v * alpha_v +\
        ...
        if np.remainder(self.nIter, loss_remainder) == 0:
            ...
            log_str = str(self.optimizer_name) + ' Iter ' + str(self.nIter) + ' Loss ' +\
                ...
                ' loss_Gamma ' + str(loss_Gamma) + ' loss_gamma ' + str(loss_gamma) + ' loss_v ' + str(loss_v) + ' LR ' +\
