...
class PINN(nn.Module):
    ...
    # 设置网络是否可导
    def change_psi_grad_state(self, grad=True):
    ...
    # omega区域损失
    def loss_omega(self, use_loss_J=True):
        x1 = self.X_omega[:, 0:1]
        x2 = self.X_omega[:, 1:2]
        # 先计算v
        v = self.net_v(x1, x2)
    ...
    # 训练一次
    def optimize_one_epoch(self, use_loss_J=True):
    ...
    def train_Adam(self, optimizer, Adam_scheduler, nIter, psiIter, vIter):
        self.optimizer = optimizer
        self.optimizer_name = 'Adam'
        self.scheduler = Adam_scheduler
        for it in range(nIter):
            for psi_i in range(psiIter):
                # 训练psi网络
                self.change_psi_grad_state(True)
                self.change_v_grad_state(False)
                self.optimize_one_epoch(use_loss_J=False)
                self.optimizer.step()
                if self.scheduler is not None:
                    self.scheduler.step()
            for v_i in range(vIter):
                # 训练v网络
                self.change_psi_grad_state(False)
                self.change_v_grad_state(True)
                self.optimize_one_epoch(use_loss_J=True)
                self.optimizer.step()
                if self.scheduler is not None:
                    self.scheduler.step()
