...
class PINN(nn.Module):
    def __init__(self, param_dict, train_dict):
        ...
        self.G1, self.G2, self.a, self.b, self.w, self.alpha, self.mu, self.ls, self.f, self.g0, self.g1, self.psi0, X_Region, X_Gamma, X_gamma, X_init = self.unzip_train_dict(
    ...
    def loss_region(self):
        x1 = self.X_Region[:, 0:1]
        x2 = self.X_Region[:, 1:2]
        t = self.X_Region[:, 2:3]
        psi = self.net_psi(x1, x2, t)
    ...
    def optimize_one_epoch(self):
        ...
        self.loss = loss_region * alpha_region + \
        ...
        if np.remainder(self.nIter, loss_remainder) == 0:
            ...
            log_str = str(self.optimizer_name) + ' Iter ' + str(self.nIter) + ' Loss ' +\
                str(loss) + ' loss_region ' + str(loss_region) + ' loss_Gamma ' + str(loss_Gamma) +\
                ' loss_gamma ' + str(loss_gamma) + ' loss_init ' + str(loss_init) + ' LR ' +\
