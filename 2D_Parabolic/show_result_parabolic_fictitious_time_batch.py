...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        PRED_T = float(sys.argv[1])
    else:
        PRED_T = float(1)

    start_time = time.time()
    # 加载各个区域的坐标数据
    TIME_STR = '20220517_091050'
    ...
    data = scipy.io.loadmat(root_path + '/' +
                            TASK_NAME + '/' + TIME_STR + '/true_T='+str(PRED_T)+'.mat')
    u_true = data['u']
    u_true = u_true.reshape((X.size, Y.size))

    data = scipy.io.loadmat(root_path + '/' +
                            TASK_NAME + '/' + TIME_STR + '/pred_T='+str(PRED_T)+'.mat')
    u_pred = data['u']
    u_pred = u_pred.reshape((X.size, Y.size))
    file_name = root_path + '/' + TASK_NAME + \
        '/' + TIME_STR + '/heatmap3_T='+str(PRED_T)
